...
两种策略都使用一份 Controller Pack 和一个控制任务。Adaptive 在同一条初始 launch
input 中额外要求 `PACK_IDENTITY_ATTESTATION`：launcher 先对本地 Pack 文件计算绝对
路径、byte length 和 SHA-256，Controller 从该路径独立复算；不得把
`codex_delegation`、XML/HTML entities、UI/read-thread wrapper 当作 Pack bytes。
...
当前回归套件共 363 项；保留并扩展 Standard/CLI 基线，并新增 Adaptive schema、显式
...
bytes，字节增长约 14.97%，低于本次 15% 预算。GitHub Actions 会在 push/PR 自动运行
同一套检查。三个示例都来自真实生成器输出。

这些是 local checks。`origin/main` 上的既有 Adaptive 仍按 `beta/experimental`
描述；当前 v3.2 分支只是 implementation candidate，只有本节本地门和 v3.2 最终真实
App E2E 都通过后才可发布。示例生成、确定性 runtime、测试、fuzz 和安装通过，不等于
每个 Codex App 版本的真实任务 loop 都能自动循环到终态。

## 真实 Codex App E2E

本仓库保留了一次在本机“测试 loop”Codex Project 中运行的受限 smoke 作为历史证据。
它曾覆盖以下场景，但不是 v3.2 的验收证据；v3.2 仍须通过自己的唯一最终受限 E2E：
...
这段历史豁免不适用于 v3.2；v3.2 未通过最终 E2E 前不能发布或声称已完整到达终态。
...
原子升级为 `status-v2`；这些修复目前只有 local
checks/fuzz 证据。完整任务身份、摘要和报告 digest 见
...
同日获授权的唯一 replacement final bounded E2E 使用新 root 和新 Controller 启动，
...
[attempt 05 evidence](evidence/v32-replacement-final-app-e2e-20260713-05.md)。
