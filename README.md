...
### v3.2.7 native Goal generation 恢复协议

当原 Controller 在 App 重启后仍是同一任务、同一 heartbeat、同一 canonical
Loop，但官方 `get_goal` 丢失原 Goal 时，新协议保持业务 Loop 暂停并 fail closed。旧
generation 只能由 runtime 重新读取唯一 ACKED GOAL CREATE 的原始 create/ACK
