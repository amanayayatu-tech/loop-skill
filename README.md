...
- 需要 Worker、Reviewer、State-Writer、Local Verifier 分工；
...
| “完成”有唯一可核验的门 | `FINALIZE_LOOP` 还不算结束；只有 `FINALIZATION_ACKED` 闭环 |
...
v3.2.8 不提供丢失 native Goal identity 后的自动恢复。当前 Codex App 没有能在同一身份下完成 create-paused、resume、restore 或 rebind 的公开接口，因此生成 Pack 不包含这条恢复流程。v3.2.7 只合入过仓库 `main`，从未创建 tag 或 GitHub Release；其停做决定由 v3.2.8 正式收口，Git 历史保持不变。
...
新生成的 Adaptive Pack 通过已安装的 MCP `runtime_codec` 传递结构化参数，覆盖 dispatch materialize/verify、正式报告与外部 receipt staging、fingerprint normalization。它不再假设 `tty:false` 子进程启动后仍会暴露可供后续 `write_stdin` 使用的 session stdin。
...
完整发布流程还包括全部 shipped Python 的 branch coverage、两组独立 5000-case fuzz、隔离安装/回滚、source/install 零漂移、安全检查和同一 exact SHA 的真实 App canary。当前主 Mac 的结构化 receipt 使用 `evidence_layer=local-main-mac`。
