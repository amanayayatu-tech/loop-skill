...
Standard/Adaptive 只是 Gate 的路线推荐，仍须经过确定性 scaffold 校验：稳定的 1 至 3
个 Goal 通常推荐 `STANDARD_LOOP`；超过 3 个里程碑、计划随证据变化、需要浏览器/本机/
设备验证、多阶段验收或预计超过半天时推荐 `ADAPTIVE_LOOP`。清晰的单步低风险任务推荐
直接完成，不应强行套 Loop。
...
半天内能收敛的工作。旧输入仍默认 Standard；两个 usage fixture 保持字节级兼容，
两个 Controller Pack fixture 因 v3.2 协议文本更新而同步更新。
...
2026-07-13 v3.2 release 本地验证记录：

- 正常完整 discovery：`Ran 363 tests in 254.742s`，`OK`。
- 生成器与 state runtime 双 5000 fuzz：`Ran 363 tests in 1282.349s`，`OK`。
...
当前回归套件共 380 项，其中 17 项覆盖 Intake Gate 静态合同、真实路径一致性和
Standard/Adaptive `--check-only` 兼容；其余保留并扩展 Standard/CLI 基线、Adaptive schema、显式
...
安装、旧备份迁移、缓存排除和安装失败回滚。v3.2 协议文本使两个 Standard Controller
...
升级前 Adaptive Pack 是 2084 行、180266 bytes；v3.2 fixture 是 2601 行、207257
bytes，字节增长约 14.97%，低于本次 15% 预算。GitHub Actions 的 Test workflow 会在
push/PR 检查 patch whitespace、编译脚本与 tests、source validator、installer syntax，
并在两个 fuzz 环境变量均为 5000 时运行 semantic regression suite。三个示例都来自
真实生成器输出。
