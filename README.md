...
2026-07-13 v3.2 核心协议在 Intake Gate 合入前的本地验证快照：

- 当时的正常完整 discovery：`Ran 363 tests in 254.742s`，`OK`。
- 当时的生成器与 state runtime 双 5000 fuzz：`Ran 363 tests in 1282.349s`，`OK`。
- source/installed validator、source/installed official `quick_validate.py`、
  `compileall`/`py_compile`、installer syntax、正式 `scripts/install.sh` 安装和
  `git diff --check`：全部 PASS。
- Standard/Adaptive fixture determinism、hash 和 size regression：PASS；source 与
  installed skill 排除 caches 后 byte-identical。

当前回归套件共 404 项，其中 21 项覆盖 Intake Gate 静态合同、真实路径一致性和
Standard/Adaptive `--check-only` 兼容，6 项固定版本、双语文档、CI、coverage 和测试拆分
发布合同；其余保留并扩展 Standard/CLI 基线、Adaptive schema、显式
...
Adaptive 终态可达路由预算下限、repair freshness 与 parent artifact 精确绑定、
...
分别提高到 5000 轮。v3.2 还覆盖 Steering 跨 ID 去重、message/cursor identity、
safe-point pause/resume、Decision stale/capability、required review-surface acceptance、
Decision PENDING 时暂停同一 heartbeat 且不阻塞原生 Goal、resume/route 前原生 Goal
真实状态对账与 `NATIVE_CONTROLLER_GOAL_IDENTITY_LOST` 停机、
投影内容 digest、Validation evidence 归档绑定、exact Worker artifact 和
Goal/dispatch/artifact-bound Context Freshness、review-surface realpath/symlink confinement、
exact report/artifact-bound `RELOAD_SAFE`、diff-bound thrashing 与
`STRATEGY_EXHAUSTED`，以及 `STATUS.md` 的
`status-v1` 兼容验证、`status-v2` 原子升级和伪造旧 digest 零副作用拒绝。它还会逐字节
核对两个 Standard fixture 组和第三个 Adaptive fixture；在隔离 `CODEX_HOME` 中验证
安装、旧备份迁移、缓存排除和安装失败回滚。v3.2.1 repair 默认值使两个 Standard Controller
Pack fixture 的哈希有意更新，两个 usage fixture 的哈希保持不变；当前四个精确基线为：

- `01` Controller Pack：`3b67094bbee2fe80a7b5c1414045072f37272c70f37287f2c099d2728ab129a2`
- `01` usage：`d8e38f4680a47aed114adf3ddfa20ba9534be7dbfc03fd6a770b345b118f81e4`
- `02` Controller Pack：`ebc01de7cef55a157dfc6a2b7a62bc2333f88f92863bd133eb722b50f4b5f81c`
- `02` usage：`cea1c0a82898712685aac818ef3862fe0cbda444967a7e0313592b77ac2eb73a`

升级前 Adaptive Pack 是 2084 行、180266 bytes；v3.2.2 fixture 是 2613 行、213556
bytes，相对 v3.2 增长约 1.10%，低于本次 2% 预算。GitHub Actions 的 Test workflow 会在
pull request、`main` push、版本 tag 检查 patch whitespace、编译脚本与 tests、source
validator、installer syntax。功能分支仅由 pull request 触发，避免同一提交产生重复或
被取消的 push check；pull request 与 `main` 另外运行 Python 3.13 双 5000 fuzz、branch
coverage 基线和隔离 `CODEX_HOME` 的 macOS 安装 smoke。版本 tag 只运行 Python 3.9
快速回归，保留发布后校验。三个示例都来自真实生成器输出。

这些是 local checks；示例生成、确定性 runtime、测试、fuzz 和安装通过，不等于每个
Codex App 版本的真实任务 loop 都能自动循环到终态。真实 App bounded smoke 与更高层
claim boundary 单独记录如下。
