...
### v3.2.7 native Goal generation recovery 暂缓

本版本不提供丢失 native Goal generation 的恢复功能。生成 Pack 不再包含恢复步骤；standalone runtime CLI 与 MCP route bridge 对旧恢复请求统一返回 `NATIVE_GOAL_GENERATION_RECOVERY_UNAVAILABLE`，`side_effects=NONE`。旧 canonical state 字段和既有 BLOCKED receipt 仅保留为历史审计证据，不能升级为 PASS。

当 required 模式发现 `NATIVE_CONTROLLER_GOAL_IDENTITY_LOST` 时，必须保持 canonical 不变、同一 heartbeat PAUSED、停止业务路由，不得创建或替换 Goal、Controller、thread、session、State-Writer 或 heartbeat。

### v3.2.6 解释器身份热修

生成的 Adaptive Pack 现在只从已安装 `codex-loop-state` MCP registration 的精确
`command`/`args` readback 解析 `RUNTIME_PYTHON` 与同一安装根下的 runtime。身份、依赖或
路径不一致时返回零副作用 `STATE_RUNTIME_UNAVAILABLE`，不再退回 ambient `python3`。
