...
CI 为所有 push 提供 Python 3.9 快速回归；pull request 与 `main` 另外运行 Python 3.13
双 5000 fuzz、branch coverage 基线和隔离 `CODEX_HOME` 的 macOS 安装 smoke。这样避免
分支 push 重复执行完整门禁，同时保留合并与发布所需的强验证。三个示例都来自真实生成器输出。
