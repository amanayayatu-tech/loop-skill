...
for required_file in "$STATE_RUNTIME" "$STATE_MCP" "$NATIVE_GOAL_OBSERVER" "$MCP_CONFIG_HELPER" "$INSTALL_VERIFY" "$APP_CANARY_VERIFY" "$STATE_SCHEMA" "$MUTATION_SCHEMA" "$INSTALL_SCHEMA" "$APP_CANARY_SCHEMA"; do
