...
- Schema v3 is host-cooperative, not Byzantine: bind real App task/automation return values and readback to the current host-attested turn, but never claim a provider-signed subtool receipt the host does not expose. `REGISTER_TASK` accepts the returned task identity; `REGISTER_HEARTBEAT` and `RECORD_HEARTBEAT_OBSERVATION` bind actual automation create/readback; `RECORD_ROUTE_SENT` accepts the returned target thread; `ACK_TRANSPORT_PAUSE` / `ACK_FINALIZATION` require an actual PAUSED readback; and `ACK_TRANSPORT_RECOVERY` requires an actual ACTIVE readback for the same heartbeat after the retained outbox resolves. A future non-argument `x-codex-app-action-receipt-v1` carrier is optional stronger evidence and is strictly verified when present, but its absence is not a normal-path blocker. Never invent a return value from a transcript, and never claim Byzantine resistance to a Controller able to forge every App call.
...
- At a matching transport fault, retain the same outbox. After the Gateway returns `WAITING_TRANSPORT_RECOVERY`, use the real App to pause the one business heartbeat, submit `ACK_TRANSPORT_PAUSE`, and notify the user once. Do not reactivate it until that same outbox is completed/recovered; then use one real ACTIVE update/readback and `ACK_TRANSPORT_RECOVERY` with the exact `active_automation_receipt` field set above. On rejection, follow the returned post-state action: re-pause only when canonical is still WAITING/PAUSED; never pause an already HEALTHY/RUNNING recovery. Read/reconcile an unknown state and do not route. Never restart Codex or create a parallel Supervisor workaround.
...
- In schema v3, register a card only with Gateway `REGISTER_DECISION={{decision_id,valid_for_state_versions,options,scope,exclusions}}`; Gateway derives source version and context digest. After one real user response, call `RECORD_DECISION_RESPONSE={{decision_id,option_id,response_text,summary,classification_reason}}` in that Controller turn; Gateway derives the turn-cursor Steering identity and normalized digest. Never use legacy Decision mutations or edit `pending_decisions`. A browser review URL may change only its port on the same explicit loopback host/scheme/path.
...
{dashboard_line}
- 正常慢：同一个 SENT outbox 有活跃 Worker/Reviewer 或新证据；只观察，不重复派发。传输退化：绑定当前 host turn 和已登记 heartbeat 的真实 observation 记录同一指纹、outbox、时间和自然-heartbeat 身份；两次自然 heartbeat 或累计 15 分钟后进入 `WAITING_TRANSPORT_RECOVERY`，仅在真实 pause 后读回 PAUSED 才投影 heartbeat PAUSED 并通知用户一次。原 outbox 恢复后，还必须对同一 heartbeat 做真实 ACTIVE update/readback 并由 `ACK_TRANSPORT_RECOVERY` 原子恢复 RUNNING；若 ACK 拒绝，必须立即将同一 heartbeat 重新 PAUSED/readback，期间禁止路由。真正终态：只有 `FINALIZATION_ACKED` 或经证据支持的 `LOOP_BLOCKED`，旧终态不可复活。
