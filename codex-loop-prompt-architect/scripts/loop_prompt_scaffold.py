    ...
    if worker["permission"] == "state_write_only":
        if adaptive:
            return (
                ...
                "- Every ACQUIRE_LEASE and TAKEOVER_LEASE includes controller_turn_id derived from the one current real Codex App turn identity. Runtime consumes it on the first acquired route and rejects a second route lease from that same App turn, even after the first route is released or completed. Never invent a new controller_turn_id to continue routing inside one turn.\n"
                "- Before each metered external model call or authorized Local Verification, stage one sanitized immutable STARTED receipt with `--external-receipt-stage`; after the call, stage the bound COMPLETED receipt before emitting stdout. If deferred execution loses stdout, recover the COMPLETED file from `.codex-loop/external-receipts/`. If only STARTED exists, conservatively count one call with unknown result/tokens and do not retry without fresh authorization. Receipts never contain prompts, responses, keys, or secrets.\n"
