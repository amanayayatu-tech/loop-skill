...
def adaptive_runtime_handoff_block() -> str:
    return (
        ...
        "- Controller and every receiving formal task use the installed `codex-loop-state` MCP "
        "`runtime_codec` tool as the sole dispatch payload codec. Controller invokes "
        "`MATERIALIZE_DISPATCH` with one strict JSON specification; the receiver invokes "
        "`VERIFY_DISPATCH` with CANONICAL_REPO_ROOT and the exact received codexDelegation.input string. "
        "Neither side implements the digest algorithm in prose. If the tool is absent, stop with "
        "RUNTIME_CODEC_TOOL_UNAVAILABLE and zero side effects.\n"
...
def heartbeat_prompt_block(
    ...
    active_worker_instruction = (
        f"Apply the deterministic transition table idempotently. If a state request lacks ACK, return WAITING_STATE_ACK and send nothing else. If a dispatch is PREPARED but not SENT, inspect the target task for its dispatch_id before any resend. If a Worker is active with progress newer than {active_stale_after_minutes} minutes, renew the exact same-owner claim with attached Controller read evidence before or after TTL when needed; atomically rebind only the same PREPARED/SENT record, record WAITING_ACTIVE, keep this heartbeat active, and never resend the dispatch. If that exact target later completes under an expired claim, perform the same renewal and ACK its existing report with the renewed claim. Probe a stale Worker at most once. Require each Worker/Reviewer/Local target task to stage its own report through `runtime_codec(operation=STAGE_REPORT, root=CANONICAL_ROOT, request=...)` before replying. Accept and forward only its ASCII-safe FORMAL_REPORT_STAGED source_path/digest/result handle to State-Writer; never read or transport the formal REPORT bytes. Wait for ACK before review, repair, next Goal, or closeout."
        if adaptive
    ...
    *,
    ...
    if worker["permission"] == "read_only":
        if adaptive:
            return (
                ...
                "- runtime-only spool: installed runtime_codec `STAGE_REPORT` may write "
    ...
    if adaptive:
        product_scopes.extend(
            [
                "RUNTIME-ONLY: installed runtime_codec STAGE_REPORT may write "
                f"{control_plane_root(audit_paths)}/report-staging/**",
...
def state_permission_text(worker: dict[str, Any], adaptive: bool = False) -> str:
    ...
    if adaptive and worker["permission"] == "read_only":
        return "product read-only; only installed runtime_codec STAGE_REPORT may write runtime-owned report-staging"
    if adaptive:
        return "product writes only in allowed scope; only installed runtime_codec STAGE_REPORT may write runtime-owned report-staging"
