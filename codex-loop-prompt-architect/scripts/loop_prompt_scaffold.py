...
- After Turn B ends, the out-of-band observer records the exact single create invocation and stable EOF without writing to the target Controller rollout. Turn C is a separate real App turn containing only get_goal. After Turn C ends, the out-of-band observer records the active same-thread readback at a later rollout offset.
