...
- Turn D is a fourth real App turn. Acquire the NATIVE_GOAL_GENERATION_COMMIT recovery lease and send COMMIT_NATIVE_GOAL_GENERATION_MIGRATION to the existing State-Writer. Runtime requires distinct A/B/C/D turn identities, create-before-readback ordering on one rollout, exact objective, changed createdAt, unchanged roles/Pack/protected state/heartbeat, and valid lost-stdout adoption. COMMIT keeps canonical and heartbeat PAUSED.
