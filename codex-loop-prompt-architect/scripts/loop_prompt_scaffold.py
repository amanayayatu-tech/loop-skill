...
- Turn D is a fourth real App turn. Acquire the NATIVE_GOAL_GENERATION_COMMIT recovery lease and send COMMIT_NATIVE_GOAL_GENERATION_MIGRATION to the existing State-Writer. Immediately before standalone runtime, that State-Writer recaptures the Phase C readback at the target Controller's current stable EOF; only strict route_state_mutation/send_message_to_thread wrappers may follow the readback. Runtime rejects NATIVE_GOAL_ROLLOUT_FINAL_EOF_CHANGED if any later byte appears, and requires distinct A/B/C/D turn identities, create-before-readback ordering, exact objective, changed createdAt, unchanged roles/Pack/protected state/heartbeat, and valid lost-stdout adoption. COMMIT keeps canonical and heartbeat PAUSED.
