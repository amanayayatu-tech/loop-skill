...
Each extracted child prompt is self-contained. Executable Workers receive the full retry ladder; Reviewer receives exact-artifact rules. Standard and retained schema-v1/v2 Adaptive State-Writers use their legacy CAS/idempotency protocol. New schema-v3 Packs create no State-Writer task: installed MCP `state_gateway({root, request})` is the sole canonical writer, resolves only the registered `codex-loop-state` runtime, and archives a root-confined Pack source path/digest without inline content, Base64, wrapper decoding, or entity replacement.
Adaptive Worker/Reviewer/Local reports never cross App transport inline: the target calls `runtime_codec` `STAGE_REPORT` with `{outbox_id,result,report_text}` and, for new Worker validation files, `evidence_sources=[{path,source_path,digest,media_type}]`; runtime reads those bytes only from the registered target worktree, stages immutable evidence, strictly frames and hashes the exact report, and supplies the confined ACK handle. `ACK_ROUTE_RESULT` or `REPORT_RECOVERY` archives the staged evidence atomically with the report on the original outbox. Controller never reads/transports/hashes report or validation bytes and must never reuse send evidence as validation evidence. When that recovered outbox was the retained `WAITING_TRANSPORT_RECOVERY` outbox, update/read back the same heartbeat as ACTIVE and use `ACK_TRANSPORT_RECOVERY`; only that Gateway CAS restores RUNNING, never a legacy run-control mutation. If that ACK rejects after the App update, immediately pause/read back the same heartbeat as directed by `PAUSE_SAME_HEARTBEAT_AND_READBACK`; routing remains forbidden. Generated Packs use typed MCP codec materialize/verify/stage/external-receipt/fingerprint operations. Legacy CLI is bounded one-frame strict-UTF-8 compatibility only; no Pack may assume a non-PTY shell will later accept `write_stdin`. Missing codec is `RUNTIME_CODEC_TOOL_UNAVAILABLE` with zero side effects.
The generated recovery call uses exactly `parameters={active_automation_receipt:{automation_id,status,automation_name,kind,target_thread_id,rrule,prompt_digest,prompt_normalization,observed_at}}`, with `status=ACTIVE`, `kind=HEARTBEAT`, and the registered heartbeat identity. Gateway derives the current source turn. On rejection, obey the post-state action with routing forbidden: WAITING/PAUSED requires immediate `PAUSE_SAME_HEARTBEAT_AND_READBACK`; HEALTHY/RUNNING returns `READ_STATE_ALREADY_RECOVERED` and must not pause; unreadable state requires read/reconciliation first.
New schema-v3 routes are `PREPARE_ROUTE -> one real App send -> RECORD_ROUTE_SENT -> role STAGE_REPORT -> ACK_ROUTE_RESULT`. Gateway atomically binds host-attested Controller turn, lease, snapshot, digest, target, nonce, and outbox. After a target-attested stage, the runtime persists a read-only target-stage sidecar derived from the SENT outbox and report digest; a different Controller MCP bridge derives and validates it, never accepts it as a public parameter, and can recover the original outbox after lost stdout or bridge separation. Send returns and automation readbacks are host-cooperative operational evidence—not provider-signed receipts and never PASS by themselves. Optional `x-codex-app-action-receipt-v1` is strict when present but never blocks normal v3 routing/pause/finalization/release/successor flow. This prevents accidental replay/mismatch and crash duplication, not a Byzantine Controller that can forge every App call. Installation is one `codex-loop-state` with absolute Python/bridge, atomic rollback/readback and zero-drift manifest; conflict fails closed. Release requires current-main-Mac same-SHA `local-main-mac` evidence and `FINALIZATION_ACKED`, not GitHub/synthetic/historical PASS. Native Goal recovery remains `NATIVE_GOAL_GENERATION_RECOVERY_UNAVAILABLE`; STATUS uses live readback; resume needs PAUSED and routing ACTIVE; execution-unstarted BLOCKED needs runtime-approved blocker_code.
Schema-v3 Decision Cards use only Gateway `REGISTER_DECISION` and `RECORD_DECISION_RESPONSE`. Registration accepts the bounded card shape but derives source state and context digest from canonical; response handling derives its Steering id, current host-attested turn cursor, normalized response digest, and stored decision-context digest. Never call legacy Decision mutations or hand-write `pending_decisions`. A browser review may substitute only the port of the same explicit loopback host/scheme/path; every other URL change is stale or mismatched.
...
Native Goal generation recovery implementation details are retained only in Git history and legacy state compatibility. Generated Packs expose only the fail-closed unavailable status.
