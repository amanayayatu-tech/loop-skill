...
Adaptive Worker/Reviewer/Local reports never cross App transport inline: each target task invokes installed `adaptive_state_runtime.py --root CANONICAL_ROOT --report-stage` before replying and returns only its ASCII-safe `FORMAL_REPORT_STAGED` handle. Controller forwards the helper-produced root-confined `.codex-loop/report-staging/` source path, media type, digest, and ACK-ready result; it never reads REPORT bytes, hand-writes staging, or computes report digests in prose. Runtime stdin is bounded to 30 seconds/4 MB/strict UTF-8 and completes on one full JSON frame without waiting for EOF; strict parsing rejects duplicate keys, non-finite numbers, extra frames, and trailing garbage. Every runtime mode uses direct argv and non-PTY (`tty:false`): launch the runtime itself before writing one compact frame, never place `dd`, `stty`, a fixed-byte reader, heredoc, shell wrapper, pipeline, or any stdin helper in front of it. Transport validation evaluates executable clauses, so a negative phrase cannot excuse a later unsafe command; only a code fence explicitly labeled `non-executable` may contain an inert unsafe example. Poll only the same yielded session. Success requires exit 0, no live session, and exactly one JSON runtime response.

New Adaptive Packs bind every post-initialize mutation to canonical `controller_pack_digest`. Controller invokes `ACQUIRE_LEASE` and `TAKEOVER_LEASE` only through the installed `route_state_mutation` MCP tool and must omit controller_turn_id; the bridge validates Codex-injected `CODEX_MCP_REQUEST_META` plus its direct OpenAI-signed app-server parent before injecting the real turn id. These two mutations never go through State-Writer, CLI, shell, or inline Python; every other mutation still goes through the existing State-Writer. JSON, argv, environment variables, task titles, and model-generated UUIDs are never attestation. Missing or mismatched metadata returns `BLOCKED_BY_APP_ATTESTATION` with zero canonical side effects. Every metered external call uses the existing LOCAL outbox `external_call_authorization` and immutable sanitized `STARTED`/`COMPLETED` receipts under `.codex-loop/external-receipts/`, binding route, provider/model, request/call index, target, timestamps, status/exit, read-only artifact, and usage; a replayed COMPLETED recovers without retry, while a lone STARTED returns `EXTERNAL_CALL_OUTCOME_UNKNOWN` and forbids retry. Digest mismatch responses use only provenance-bearing `provided_digest/computed_digest`, `ledger_digest/computed_file_digest`, `state_digest/mutation_digest`, or `canonical_pack_digest/loaded_pack_digest` pairs plus algorithm, encoding, byte length, and `side_effects=NONE`; never infer direction from `expected/actual`. A changed Pack is inert until explicit `MIGRATE_CONTROLLER_PACK` at a paused safe point. A Worker control-plane `BLOCKED` result avoids repair consumption only when it declares top-level `execution_started=false` with a top-level runtime-approved deterministic `blocker_code`; report staging binds both into the ACK-ready result. Historical unclassified results preserve legacy accounting. If an older ACK dropped that explicit classification, use `RECONCILE_WORKER_EXECUTION_CLASSIFICATION` only at a canonical paused safe point with no lease/active outbox; runtime verifies the immutable archived report and changes only the existing attempt classification.
