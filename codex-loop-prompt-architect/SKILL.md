...
Adaptive Worker/Reviewer/Local reports never cross App transport inline: each target task invokes installed `adaptive_state_runtime.py --root CANONICAL_ROOT --report-stage` before replying and returns only its ASCII-safe `FORMAL_REPORT_STAGED` handle. Controller forwards the helper-produced root-confined `.codex-loop/report-staging/` source path, media type, digest, and ACK-ready result; it never reads REPORT bytes, hand-writes staging, or computes report digests in prose. Runtime stdin is bounded to 30 seconds/4 MB/strict UTF-8 and completes on one full JSON frame without waiting for EOF. Every runtime mode uses direct argv and non-PTY (`tty:false`): launch the runtime itself before writing one compact frame, never place `dd`, `stty`, a fixed-byte reader, heredoc, shell wrapper, pipeline, or any stdin helper in front of it. Poll only the same yielded session. Success requires exit 0, no live session, and exactly one JSON runtime response.

New Adaptive Packs bind every post-initialize mutation to canonical `controller_pack_digest`, every route lease/takeover to one real `controller_turn_id`, and every metered external call to immutable sanitized `STARTED`/`COMPLETED` receipts under `.codex-loop/external-receipts/`. A changed Pack is inert until explicit `MIGRATE_CONTROLLER_PACK` at a paused safe point. A Worker control-plane `BLOCKED` result avoids repair consumption only when it declares top-level `execution_started=false` with a top-level runtime-approved deterministic `blocker_code`; report staging binds both into the ACK-ready result. Historical unclassified results preserve legacy accounting. If an older ACK dropped that explicit classification, use `RECONCILE_WORKER_EXECUTION_CLASSIFICATION` only at a canonical paused safe point with no lease/active outbox; runtime verifies the immutable archived report and changes only the existing attempt classification.
