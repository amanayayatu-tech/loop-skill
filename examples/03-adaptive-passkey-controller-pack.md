...
- Turn B is a new real App turn. Re-read PREPARED/get_goal, then run installed adaptive_state_runtime.py --native-goal-observe by direct argv, tty:false, one compact frame, canonical rollout path, stored offset/digests, and a confined report path. Continue only on stable_eof=true, matching_invocation_count=0, invocation_state=NONE, get_goal=null, and PAUSED heartbeat.
- Then call official create_goal exactly once with unchanged historical objective bytes: `const result = await tools.create_goal({"objective":<JSON_STRING_OF_EXACT_BYTES>}); text(JSON.stringify(result));`. Include token_budget only when its exact historical authorization exists; send no State-Writer mutation, business route, provider call, or second create. Before replay, observe from the stored offset: Any matching or ambiguous create, including STARTED_UNKNOWN, forbids another create. Started plus null is NATIVE_GOAL_CREATE_OUTCOME_UNKNOWN; lost stdout allows later adoption only, never retry/rollback/replacement.
- Turn C is a third real App turn. Exact one-invocation rollout plus active same-thread get_goal authorizes NATIVE_GOAL_GENERATION_COMMIT and COMMIT_NATIVE_GOAL_GENERATION_MIGRATION. Runtime validates objective, createdAt change, A/B/C turns, roles, Pack, protected state, heartbeat, usage, and adoption. COMMIT keeps canonical and heartbeat PAUSED.
- NATIVE_GOAL_GENERATION_ROLLBACK requires zero invocation since PREPARE, two newer distinct-turn null receipts, and unchanged protected state. Any possible invocation forbids it; success restores journaled generation/heartbeat routing state and preserves history.
