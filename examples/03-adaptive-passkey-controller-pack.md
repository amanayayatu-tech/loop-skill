...
- Every metered external call/Local Verification requires one canonical LOCAL outbox `external_call_authorization` binding receipt_id, action_kind, provider, model, request_digest, call_index, and a confined result artifact_path. Its STARTED/COMPLETED receipts under `.codex-loop/external-receipts/` additionally bind loop, Pack, Goal, local dispatch/outbox, lease/routing turn, target role/thread, timestamps, status, exit code, immutable artifact digest, and usage. Stage STARTED before send and COMPLETED before stdout. Replaying COMPLETED recovers without provider retry; replaying a lone STARTED returns EXTERNAL_CALL_OUTCOME_UNKNOWN and forbids retry. Unknown tokens stay null with complete=false. Receipts never contain prompts, responses, keys, Authorization, or secrets.
...
- Formal report artifacts are never inline. Accept only the ASCII-safe FORMAL_REPORT_STAGED handle produced inside the Worker/Reviewer/Local target task by `adaptive_state_runtime.py --root CANONICAL_ROOT --report-stage`; its source_path must be a helper-generated root-confined non-canonical `.codex-loop/report-staging/` regular non-symlink read-only JSON file whose outbox-bound filename, digest, media type, and ACK-ready result all match. Never accept a Controller-written staging file or raw REPORT bytes relayed through App transport. A Worker BLOCKED report includes top-level execution_started; only execution_started=false plus a top-level runtime-approved deterministic control-plane blocker_code preserves history without consuming repair budget. Report staging binds both fields into its ACK-ready result. If an older ACK dropped them, RECONCILE_WORKER_EXECUTION_CLASSIFICATION is allowed only at PAUSED_AT_SAFE_POINT with no lease/active outbox and exact archived report identity; it corrects no other history. ASSURANCE RECORD_REVIEW has zero artifacts and its ACK path; runtime reopens it.
...
- Every metered external model call or authorized Local Verification requires one canonical LOCAL outbox `external_call_authorization` binding receipt_id, action_kind, provider, model, request_digest, call_index, and a confined result artifact_path. STARTED/COMPLETED under `.codex-loop/external-receipts/` additionally bind loop, Pack, Goal, local dispatch/outbox, lease/routing turn, target role/thread, timestamps, status, exit code, immutable read-only artifact digest, and usage. Stage STARTED before send and COMPLETED before stdout. Replayed COMPLETED recovers without provider retry; a lone/replayed STARTED returns EXTERNAL_CALL_OUTCOME_UNKNOWN and forbids retry. Unknown tokens stay null with complete=false. Receipts never contain prompts, responses, keys, Authorization, or secrets.
