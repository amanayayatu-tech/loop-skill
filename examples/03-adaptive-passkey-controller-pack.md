...
- Use only after an APPLIED NATIVE_GOAL_GENERATION_RECOVERY_AUTHORIZED Correction, canonical and the same heartbeat PAUSED, unchanged five roles, and no lease or route-reserving outbox; this is not general Goal recreation.
- v3.2.7 migration builds the runtime-derived legacy generation baseline from the unique ACKED GOAL CREATE create/ACK bytes. Controller and State-Writer supply no historical identity, objective, createdAt, usage, call count, or digest.
- Controller obtains one signed-MCP recovery ACQUIRE_LEASE per real App turn. Only the original State-Writer consumes it for standalone-runtime PREPARE, COMMIT, or ROLLBACK and returns runtime JSON.
- Turn A uses NATIVE_GOAL_GENERATION_PREPARE plus PREPARE_NATIVE_GOAL_GENERATION_MIGRATION. It binds two distinct-turn get_goal=null receipts, PAUSED heartbeat, source/target generation, exact objective, active Pack/authorization, rollout high-watermark, and one AUTHORIZED_UNUSED create outbox; ACK consumes the lease and ends the turn.
- Before Turn B, a trusted read-only observer outside the target Controller rollout runs installed `adaptive_state_runtime.py --native-goal-observe` and requires stable_eof=true, matching_invocation_count=0, invocation_state=NONE, get_goal=null, and the same PAUSED heartbeat. If no such out-of-band observation path exists, stop with an upstream blocker; never run the observer from the target Controller turn or add its tool call to that rollout.
- Turn B is a new real App turn containing exactly one official create_goal call with unchanged historical objective bytes: `const result = await tools.create_goal({"objective":<JSON_STRING_OF_EXACT_BYTES>}); text(JSON.stringify(result));`. Never include token_budget. Send no get_goal, observer, State-Writer mutation, business route, provider call, or second create in this turn. Any matching or ambiguous create, including STARTED_UNKNOWN, forbids another create, rollback, or replacement; an exact create with a different objective is ambiguous. Lost stdout permits later adoption only.
- After Turn B ends, the out-of-band observer captures the exact single create invocation at the then-current stable EOF without writing to the target Controller rollout; callers cannot choose an older cutoff. Turn C is a separate real App turn containing only get_goal. After Turn C ends, the observer captures the active same-thread readback at a later EOF. Runtime replays the earlier prefix by bound snapshot digest and requires the later observation to cover every intervening byte; any hidden second or ambiguous create blocks COMMIT.
- Turn D is a fourth real App turn. Acquire the NATIVE_GOAL_GENERATION_COMMIT recovery lease and send COMMIT_NATIVE_GOAL_GENERATION_MIGRATION to the existing State-Writer as one compact canonical JSON handoff. Immediately before standalone runtime, that State-Writer recaptures the Phase C readback at the target Controller's current stable EOF. One stable snapshot must supply both Goal identity and the complete create window; runtime must not reopen the rollout between those decisions. The suffix begins exactly at the readback turn end. Its one route plus one State-Writer handoff must bind canonical scope, migration, lease, attested turn, target thread, and handoff digest; name-only or caller-shifted boundaries are forbidden. Runtime rejects NATIVE_GOAL_ROLLOUT_FINAL_EOF_CHANGED if any later byte appears, and requires distinct A/B/C/D turn identities, create-before-readback ordering, exact objective, changed createdAt, unchanged roles/Pack/protected state/heartbeat, and valid lost-stdout adoption. COMMIT keeps canonical and heartbeat PAUSED.
- NATIVE_GOAL_GENERATION_ROLLBACK requires zero invocation since PREPARE, two newer distinct-turn null receipts, and unchanged protected state. The State-Writer recaptures the final null receipt at current stable EOF immediately before runtime, and that same snapshot must prove the complete create window remains NONE. Any possible invocation or later byte forbids rollback; success restores journaled generation/heartbeat routing state and preserves history.
- RESUME is a later independent turn, forbidden for PREPARED/CONFLICT/OUTCOME_UNKNOWN. After COMMITTED reconciliation, SET_RUN_CONTROL(RESUME) and same-heartbeat ACTIVE readback occur in separate turns before routing. Never substitute Goal, Controller, role, thread, session, or heartbeat.
- Persist only sanitized receipts. Synthetic/source evidence is not release proof; exact-SHA current-App canary remains required.
...
| Native Goal generation migration is `PREPARED` | Keep canonical/heartbeat PAUSED; in a new turn observe from the stored high-watermark. Only zero matching invocation plus `get_goal=null` permits one exact historical-objective `create_goal` | resume, business route, combined PREPARE/CREATE/COMMIT, second create, or altered objective/budget |
...
- Construct exactly `{"envelope_type": "WORKER_DISPATCH|REVIEW_DISPATCH|LOCAL_VERIFY_DISPATCH", "payload": {...}}` with literal `PAYLOAD_DIGEST_PLACEHOLDER`. Using the bound installed interpreter, send one compact JSON frame to `[RUNTIME_PYTHON, RUNTIME_PATH, "--payload-materialize"]` via raw stdin with `tty:false`. Do not use `dd`, `stty`, fixed-byte readers, heredocs, or any extra shell pipeline. Never treat terminal echo as runtime stdout.
