...
- Turn D is a fourth real App turn. Acquire the NATIVE_GOAL_GENERATION_COMMIT recovery lease and send COMMIT_NATIVE_GOAL_GENERATION_MIGRATION to the existing State-Writer. Immediately before standalone runtime, that State-Writer recaptures the Phase C readback at the target Controller's current stable EOF. One stable snapshot must supply both Goal identity and the complete create window; runtime must not reopen the rollout between those decisions. Only a finite exact-name set for route_state_mutation/send_message_to_thread wrappers may follow the readback; suffix matches are forbidden. Runtime rejects NATIVE_GOAL_ROLLOUT_FINAL_EOF_CHANGED if any later byte appears, and requires distinct A/B/C/D turn identities, create-before-readback ordering, exact objective, changed createdAt, unchanged roles/Pack/protected state/heartbeat, and valid lost-stdout adoption. COMMIT keeps canonical and heartbeat PAUSED.
