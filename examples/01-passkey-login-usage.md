...
当前 heartbeat 总预算覆盖约 16 小时（15 分钟 x 64 次）；预计任务超过该范围时必须在启动前提高 max_wakeups。
...
1. 在 Codex App 左侧选择或创建项目工作区：`myapp`，根目录必须是 `/workspace/myapp`。
...
3. 把资料放进工作区或提供子线程可读的绝对路径：/workspace/myapp/docs/auth-spec.md, /workspace/myapp/docs/login-flow/。只附在控制聊天里的文件不会自动传给新线程。
...
10. heartbeat 每 15 分钟唤醒，最多 64 次；Worker 正在运行时记录 `WAITING_ACTIVE`，不能 NOOP 关闭。只有终态或无 inflight/queue 且连续 8 次 idle 才允许暂停。
...
- `/workspace/myapp/.codex-loop/LOOP_STATE.md`：版本化当前快照、Goal Queue、thread-creation/automation/dispatch outbox、inflight dispatch、线程登记、预算和审批 ledger。
- `/workspace/myapp/.codex-loop/LOOP_EVENTS.jsonl`：按 event_id 记录的派发、ACK、重试、审查、停止流水。
- `/workspace/myapp/.codex-loop/TRIAGE.md`：TRIAGE_ACTIONABLE/TRIAGE_NO_ACTION 发现及其证据和后续 Goal。
- `/workspace/myapp/.codex-loop/reports/`：Worker、Reviewer 和 FINAL_AUDIT 报告归档。
- `/workspace/myapp/.codex-loop/transactions/`：State-Writer 的 PREPARED/APPLIED 恢复日志；用于判断中断后缺哪一步，不能当作第二份 canonical state。
- `/workspace/myapp/.codex-loop/sources/CONTROLLER_PACK.md`：初始化时归档的精确 Controller Pack；heartbeat 校验 PACK_SHA256 后用它抵抗长对话压缩。
