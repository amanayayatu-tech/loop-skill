...
- The Controller thread must run inside the Codex Project whose root is /workspace/product-app.
- Workspace setup: Open /workspace/product-app as a Codex Project before starting; writing Workers use isolated worktrees
- Connector policy: GitHub connector if exposed; otherwise workspace CI log files and manually supplied URLs
- Resolve projectId with list_projects before child thread creation.
- Required source artifacts: /workspace/product-app/docs/ci-evidence/
...
- branch field: codex/daily-ci-triage-repair
...
- Human approval policy: Local code, tests, and configuration changes inside the allowed scope are pre-authorized. Production deploy, merge, secrets, user-data deletion, real external writes, and public/scientific claims remain human gates unless explicitly pre-approved.
...
- Worktree/integration policy: one isolated worktree for implementation; triage is read-only; Reviewer uses same-directory fork; State-Writer remains in control-plane checkout
...
- Required order:
  ...
  5. If no matching state exists, send LOOP_INITIALIZED with expected_state_version=0 through state-writer; atomically archive the exact pack at /workspace/product-app/.codex-loop/sources/CONTROLLER_PACK.md, record its PACK_SHA256/controller_pack_identity, create state version 1 including State-Writer registry identity, and wait for STATE_WRITE_APPLIED. If state exists, verify/reconcile the stored pack identity instead of overwriting it.
  6. Persist THREAD_CREATE_PREPARED for triage; wait for ACK; reconcile or create it once with BOOTSTRAP_PROMPT; persist THREAD_REGISTERED with real threadId/worktree_path and wait for ACK. Do not create Reviewer yet.
  ...
  8. Materialize First Goal placeholders, persist DISPATCH_PREPARED for triage, wait for ACK, send once, then persist DISPATCH_SENT/inflight state.
...
| --- | --- | --- | --- |
| triage | <MATERIALIZE_REAL_THREAD_ID_FOR_TRIAGE> | read_only (explicit) | read-only discovery and classification of CI failures |
| implementation | <MATERIALIZE_REAL_THREAD_ID_FOR_IMPLEMENTATION> | workspace_write (explicit) | repair one selected low-risk failure inside approved scope |
...
| --- | --- | --- | --- | --- |
| 1 | CI-T1 | triage | none | startup transaction is complete |
| 2 | CI-R1 | implementation | CI-T1 | CI-T1 status is TRIAGE_ACTIONABLE and the finding has concrete evidence, allowed scope, and validation |
...
- State: /workspace/product-app/.codex-loop/LOOP_STATE.md
- Events: /workspace/product-app/.codex-loop/LOOP_EVENTS.jsonl
- Triage: /workspace/product-app/.codex-loop/TRIAGE.md
- Reports: /workspace/product-app/.codex-loop/reports/
- Recovery journals: /workspace/product-app/.codex-loop/transactions/
- Trusted Controller Pack snapshot: /workspace/product-app/.codex-loop/sources/CONTROLLER_PACK.md
- State schema:
...
Continue this Codex Loop as its read-only Controller. Do not edit product files. Read the trusted Controller Pack snapshot at canonical controller_pack_identity.path (initially /workspace/product-app/.codex-loop/sources/CONTROLLER_PACK.md) and verify its SHA-256 against canonical controller_pack_identity.digest; use the copy in this thread only as corroboration. Then read canonical state at /workspace/product-app/.codex-loop/LOOP_STATE.md, recent events at /workspace/product-app/.codex-loop/LOOP_EVENTS.jsonl, and every registered active task before acting. Route only through real Codex App project tasks and state-writer.
...
- HEARTBEAT_AUTOMATION_NAME is the exact string `product-app loop heartbeat ` plus loop_id from canonical state. Its prompt digest is SHA-256 of the exact HEARTBEAT_PROMPT text.
...
- Cadence policy: heartbeat every 15 minutes; max 48 total wakeups; pause only after terminal completion or 8 consecutive idle wakeups with no inflight/queued work
...
| wake_count reaches 48 before terminal state | Persist HEARTBEAT_BUDGET_EXHAUSTED and STOP for explicit extension; do not claim completion | silent shutdown |


Discovery/Triage:
- Sources: GitHub Actions failures, open PR checks, recent commits, failing local tests, user triage notes
- Output: /workspace/product-app/.codex-loop/TRIAGE.md through State-Writer only.
...
SEND TO: real Codex App task for triage; Controller records the returned real threadId after create/fork

```text
Role: triage
Responsibility: read-only discovery and classification of CI failures
Repo/root: /workspace/product-app
Repo Mode: existing_git
Target Branch: codex/daily-ci-triage-repair
...
- triage: /workspace/product-app/.codex-loop/TRIAGE.md
- reports: /workspace/product-app/.codex-loop/reports/
- transactions: /workspace/product-app/.codex-loop/transactions/
- trusted pack snapshot: /workspace/product-app/.codex-loop/sources/CONTROLLER_PACK.md
...
Validation Commands:
- inspect supplied CI logs and local git evidence read-only
...
- package.json
- package-lock.json
- EXPLICIT EXCLUSION (State-Writer only): /workspace/product-app/.codex-loop/**
...
Allowed Write Scope:
- /workspace/product-app/.codex-loop/LOOP_STATE.md
- /workspace/product-app/.codex-loop/LOOP_EVENTS.jsonl
- /workspace/product-app/.codex-loop/TRIAGE.md
- /workspace/product-app/.codex-loop/reports/
- /workspace/product-app/.codex-loop/transactions/
- /workspace/product-app/.codex-loop/sources/
...
## First Goal
SEND VIA: Controller to real Worker thread for triage

```text
/goal
Goal ID: CI-T1
Dispatch ID: <MATERIALIZE_DISPATCH_ID_FOR_CI-T1>
Parent Dispatch ID: none for the first attempt; exact prior dispatch_id for a repair attempt
Phase: Read-only CI triage
Target Thread Identifier: <MATERIALIZE_REAL_THREAD_ID_FOR_TRIAGE>
Worker Role: triage
Worker Permission: read_only
Repo/root: /workspace/product-app
Repo Mode: existing_git
Target Branch: codex/daily-ci-triage-repair
Source Artifacts: /workspace/product-app/docs/ci-evidence/
Depends On: none
Dispatch When: startup transaction is complete
Objective: Inspect the declared CI evidence and select at most one concrete low-risk failure

Current Control-Plane State Snapshot:
<MATERIALIZE_CURRENT_STATE_SNAPSHOT_FOR_CI-T1>
...
Success Criteria:
- Return TRIAGE_ACTIONABLE with one finding and evidence, or TRIAGE_NO_ACTION with evidence
- Do not modify files or dispatch implementation directly
...
Canonical Control-Plane State: /workspace/product-app/.codex-loop/LOOP_STATE.md
...
### Queued Goal Template - CI-R1

```text
/goal
Goal ID: CI-R1
Dispatch ID: <MATERIALIZE_DISPATCH_ID_FOR_CI-R1>
Parent Dispatch ID: none for the first attempt; exact prior dispatch_id for a repair attempt
Phase: Scoped CI repair
...
Source Artifacts: /workspace/product-app/docs/ci-evidence/
Depends On: CI-T1
Dispatch When: CI-T1 status is TRIAGE_ACTIONABLE and the finding has concrete evidence, allowed scope, and validation
Objective: Repair the single finding selected by CI-T1 without expanding scope

Current Control-Plane State Snapshot:
<MATERIALIZE_CURRENT_STATE_SNAPSHOT_FOR_CI-R1>
Required snapshot keys: loop_id, state_version, repo/worktree identity, this Goal status, dependencies, approval ledger slice, budget ledger slice, retry/repair counters, pre-existing dirty-file boundary, and current claim/evidence limits. Keep it bounded; do not replace it with only a path.

Success Criteria:
- The selected failure is repaired inside approved files
- npm test, lint, and typecheck pass
- Independent Reviewer approves the exact Worker worktree diff
